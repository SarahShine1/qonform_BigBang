...
// ── Champ row ────────────────────────────────────────────────────────────────
function ChampRow({ champ, editMode, onDelete, onDragStart, onDragOver, onDrop, isDragging }) {
  return (
    <div
      draggable={editMode}
    ...
    >
      ...
      {editMode && (
        <>
          <button type="button" onClick={() => onDelete(champ)}
...
function SectionCard({
  section, editMode,
  onAddChamp, onDeleteChamp, onDeleteSection,
  onSectionDragStart, onSectionDragOver, onSectionDrop,
  onChampDragStart, onChampDragOver, onChampDrop,
  draggingChamp,
}) {
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3"
        ...
        {editMode && (
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => onAddChamp(section)}
              className="flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[11px] font-semibold transition hover:bg-emerald-50"
              style={{ border: "1.5px dashed #6EE7B7", color: "#065F46", backgroundColor: "transparent" }}>
      ...
      <div>
        {visibleChamps.length === 0 && (
          <p className="px-4 py-3 text-[11.5px] italic text-slate-400"
            style={{ fontFamily: "'DM Sans', sans-serif" }}>
            Aucun champ — {editMode ? "cliquez « Ajouter un champ »." : "section vide."}
          </p>
        )}
        {visibleChamps.map((champ, ci) => (
          <ChampRow
            ...
            onDragStart={() => onChampDragStart(section.id_section_template, ci)}
            onDragOver={(e) => { e.stopPropagation(); onChampDragOver(e); }}
            onDrop={(e) => { e.stopPropagation(); onChampDrop(section.id_section_template, ci); }}
            isDragging={
...
export default function NormeTemplatePage() {
  ...
  const doDeleteSection = async (section) => {
    ...
    setSections((prev) => prev.filter((s) => s.id_section_template !== section.id_section_template));
    setConfirm(null);
  };
  ...
  const handleDeleteChamp = (champ) => {
    if (champ._pending) {
      ...
      setSections((prev) =>
        prev.map((s) => ({ ...s, champs: s.champs.filter((c) => c.id_champ_template !== champ.id_champ_template) }))
      );
    } else {
    ...
    setSections((prev) =>
      prev.map((s) => ({
        ...
        champs: s.champs.map((c) =>
          c.id_champ_template === champ.id_champ_template ? { ...c, _deleted: true } : c
        ),
      }))
    );
    setConfirm(null);
  ...
  const handleSave = async () => {
    ...
    try {
      for (const section of sections) {
        ...
        for (const champ of section.champs.filter((c) => c._deleted && !c._pending)) {
  ...
  // ── Section drag ──────────────────────────────────────────────────────────
  const handleSectionDragStart = (idx) => { sectionDragIdx.current = idx; };
  ...
  const handleSectionDrop = async (targetIdx) => {
    ...
    await Promise.all(withOrdre.map((s) => updateSection(s.id_section_template, { ordre: s.ordre })));
  };
  ...
  const handleChampDragOver = (e) => { e.preventDefault(); };
  const handleChampDrop = (targetSectionId, targetIdx) => {
    ...
    if (drag.champIdx === targetIdx) { setDraggingChamp(null); return; }
    setSections((prev) =>
      prev.map((s) => {
        ...
        return { ...s, champs: [...visible.map((c, i) => ({ ...c, ordre: i + 1 })), ...hidden] };
      })
    );
    ...
    <AppLayout pageTitle="Canevas fiche" userName={userName} userRole={userRole}>
      <div style={{ fontFamily: "'DM Sans', sans-serif" }} className="pb-12">
        ...
        <div className="mb-3 flex items-center gap-1.5 text-[12px] text-slate-500">
          <button type="button" onClick={() => navigate("/cartographie/canevas-fiche")}
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            ...
            style={
              editMode
                ? { backgroundColor: "#fff", borderColor: PURPLE, color: PURPLE }
          ...
          {editMode && (
            <p className="hidden text-[11px] text-slate-400 sm:block">
              Icône rouge pour supprimer un champ · + Ajouter pour en créer un nouveau
            </p>
          )}
          ...
          {editMode && (
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={handleSave} disabled={saving || !hasPendingChanges}
                ...
                style={{ backgroundColor: PURPLE }}
                onMouseEnter={(e) => !saving && (e.currentTarget.style.backgroundColor = "#45107A")}
                onMouseLeave={(e) => !saving && (e.currentTarget.style.backgroundColor = PURPLE)}>
                {saving
          ...
          <div className="mb-4">
            {addingSect ? (
              <div className="flex flex-col gap-2">
                ...
                <button type="button" onClick={() => setAddingSect(false)}
        ...
        {loading && (
          <div className="flex items-center justify-center py-16 text-[13px] text-slate-400">
            <Loader2 size={18} className="mr-2 animate-spin" style={{ color: PURPLE }} /> Chargement…
          </div>
        )}
        ...
        {!loading && sections.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 py-16">
            <p className="text-[13px] text-slate-400">Aucune section pour cette norme.</p>
            {editMode && (
              <button type="button" onClick={() => setAddingSect(true)}
        ...
        <div className="space-y-3">
          {sections.map((section, idx) => (
            <SectionCard
              ...
              onDeleteSection={(s) => setConfirm({ type: "section", target: s })}
